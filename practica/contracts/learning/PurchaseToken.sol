...
contract MiTokenParaVenta is ERC20, AccessControl {
    ...
    function purchaseFixRate(uint256 _usdcAmount) external {
        ...
        uint256 saldoDisponible = usdc.balanceOf(msg.sender);
        // require(?, "No tiene suficiente UDSC");
        require(saldoDisponible >= _usdcAmount, "No tiene suficiente UDSC");

        // verifica que caller ha dado permiso al contrato MTPV
        // usar usdc.allowance(msg.sender, address(this))

        uint256 saldoPermitido = usdc.allowance(msg.sender, address(this));
        // require(?, "No tiene suficiente permiso");
        require(saldoPermitido >= _usdcAmount, "No tiene suficiente permiso");
    ...
    function purchaseVariableRate(uint256 _usdcAmount) external {
        ...
        uint256 balanceDisponible = usdc.balanceOf(msg.sender);
        require(balanceDisponible >= _usdcAmount, "No tiene suficiente UDSC");

        // verifica que caller ha dado permiso al contrato MTPV
        // usar usdc.allowance(msg.sender, address(this))
        // require(?, "No tiene suficiente permiso");

        uint256 balancePermitido = usdc.allowance(msg.sender, address(this));
        require(balancePermitido >= _usdcAmount, "No tiene suficiente permiso");
        ...
        returns (uint256)
    {
        ...
        // 1 USDC = 25 MTPV
        uint256 tokenTransformed = _usdcAmount * exchangeRate;
