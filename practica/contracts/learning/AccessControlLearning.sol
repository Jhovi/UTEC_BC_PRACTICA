...
contract AccessControlLearning {
    ...
    modifier onlyRole(bytes32 _role) {
        ...
        if (temporalRole[msg.sender][_role].tempRol) {
            bool intentos = temporalRole[msg.sender][_role].executions < temporalRole[msg.sender][_role].limit;
            require(intentos, "Cuenta no tiene el rol necesario");
            temporalRole[msg.sender][_role].executions = temporalRole[msg.sender][_role].executions + 1; 
            if (temporalRole[msg.sender][_role].executions == temporalRole[msg.sender][_role].limit) 
                temporalRole[msg.sender][_role].tempRol = false;
        } else {
    ...
    mapping(address => mapping(bytes32 => TempRole)) temporalRole;
    ...
    function grantRoleTemporarily(
        ...
        TempRole memory tempRole = TempRole(true, 0, _limit); 

        temporalRole[_account][_role] = tempRole;
    ...
    {
        uint256 intentosRestantes = temporalRole[_account][_role].limit - temporalRole[_account][_role].executions;
        if (!temporalRole[_account][_role].tempRol) {
            return (false, 0);
        } 
        return (temporalRole[_account][_role].tempRol , intentosRestantes);
    }
}
